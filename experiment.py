from regra.dataset.explanation_bank import TableStoreDataset  
from regra.dataset.explanation_bank import ExplanationBankDataset  
from regra.ranker.tfidf import TFIDF
from regra.ranker.bm25 import BM25
from regra.ranker.unification_score import UnificationScore
from regra.ranker.relevance_score import RelevanceScore
...
# Load table-store
ts_dataset = TableStoreDataset(config_file='./conf/prod/dirs.jsonnet', mode='train').output().load()  
# Load train and dev set
eb_dataset_train = ExplanationBankDataset(config_file='./conf/prod/dirs.jsonnet', mode='train').output().load() 
eb_dataset_dev = ExplanationBankDataset(config_file='./conf/prod/dirs.jsonnet', mode='dev').output().load() 
...
for t_id, ts in tqdm(ts_dataset.items()):
    ...
    for chunck in fact:
        temp = []
        for word in nltk.word_tokenize(chunck.replace("?"," ").replace("."," ").replace(","," ").replace(";"," ").replace("-"," ")):
...
for q_id, exp in tqdm(eb_dataset_train.items()): 
    ...
    if exp._answerKey in exp._choices:     
        question = exp._question.replace("?"," ").replace("."," ").replace(","," ").replace(";"," ").replace("-"," ").replace("'","").replace("`","")
        candidate = exp._choices[exp._answerKey].replace("?"," ").replace("."," ").replace(","," ").replace(";"," ").replace("-"," ").replace("'","").replace("`","")
    else:  
        question = exp._question.replace("?"," ").replace("."," ").replace(","," ").replace(";"," ").replace("-"," ").replace("'","").replace("`","")
        candidate = ""
...
for q_id, exp in tqdm(eb_dataset.items()):
    ...
    for t_id, ts in ts_dataset.items():
        if t_id in relevance_scores.keys() and t_id in unification_scores.keys():
            combined_scores[t_id] = (weights[0] * relevance_scores[t_id]) + (weights[1] * unification_scores[t_id])
